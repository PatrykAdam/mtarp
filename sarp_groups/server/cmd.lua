...
function cmd.pokoj(playerid, cmd, cmd2, ...)
	...
	if cmd2 == 'wejdz' then
		...
		local position = exports.sarp_doors:getDoorData(doorid, {"exitX", "exitY", "exitZ", "exitRot", "exitinterior", "exitdimension"})
                ...
                bindKey(playerid, "E", "down", function(playerid)
                    local doorElement = getElementData(playerid, "player:door")
                    local pickup = getElementData(doorElement, "doors:parent")
                    local dX, dY, dZ = getElementPosition( pickup )
                    local dRot = getElementData( pickup, "doors:posRot")
                    setElementPosition( playerid, dX, dY, dZ )
                    setElementRotation( playerid, 0, 0, dRot or 0 )
                    setElementDimension( playerid, getElementDimension( pickup ) )
                    setElementInterior( playerid, getElementInterior( pickup ) )
